...
import type { ApiResultData } from '@/app/types/messages'
    ...
    <Card className='border-border/50 bg-card/30 overflow-hidden'>
      <div className='flex items-center justify-between p-3 border-b border-border/50'>
        ...
        <div className='flex items-center gap-3 shrink-0 ml-2'>
          <span className={cn('font-mono text-sm font-medium', getStatusColor())}>{data.statusCode}</span>
      ...
      <div className='flex border-b border-border/50'>
        <button
          ...
          className={cn(
            ...
            activeTab === 'body' ? 'text-foreground border-b-2 border-emerald-500' : 'text-muted-foreground hover:text-foreground'
          )}
        >
          Response Body
        </button>
        {data.headers && (
          <button
            ...
            className={cn(
              'px-4 py-2 text-xs font-medium transition-colors',
              activeTab === 'headers' ? 'text-foreground border-b-2 border-emerald-500' : 'text-muted-foreground hover:text-foreground'
            )}
          >
            Headers
          </button>
      ...
      <div className='relative'>
        ...
        <pre className='p-3 text-xs text-foreground overflow-x-auto max-h-64 font-mono'>
          {activeTab === 'body' ? JSON.stringify(data.responseBody, null, 2) : JSON.stringify(data.headers, null, 2)}
        </pre>
      </div>
    </Card>
  )
