...
export function ResizablePanel({
  ...
  const [resizeState, dispatch] = useActionState(
    (prev: { width: number; isResizing: boolean }, action: { type: 'start'; x: number } | { type: 'move'; x: number } | { type: 'end' }) => {
      switch (action.type) {
        case 'start':
        ...
        case 'move': {
          ...
          const deltaX = resizeDirection === 'right' ? action.x - dragState.current.startX : dragState.current.startX - action.x
          const newWidth = Math.max(minWidth, Math.min(maxWidth, dragState.current.startWidth + deltaX))
  ...
  return (
    <div ref={panelRef} className={`relative shrink-0 ${className}`} style={{ width: `${width}px` }}>
      {children}
