...
export class ToolsRegistry implements OnModuleInit {
  ...
  async onModuleInit() {
    ...
    console.log('[ToolsRegistry] Initialized with', this.enabledTools.size, 'tools')
  }
  ...
  private loadCustomTools(): void {
    ...
    console.log(`[ToolsRegistry] Loading ${customConfigs.length} custom tools...`)
  ...
  private async preloadLangChainTools(): Promise<void> {
    ...
    console.log(`[ToolsRegistry] Preloading ${langChainConfigs.length} LangChain tools...`)
    ...
    for (const config of langChainConfigs) {
      ...
      try {
        ...
        if (!ToolClass) {
          console.error(`[ToolsRegistry]   Failed to find class: ${className} in ${importPath}`)
          continue
        }
  ...
  private async preloadMCPTools(): Promise<void> {
    ...
    console.log(`[ToolsRegistry] Preloading ${serverNames.length} MCP servers...`)
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => reject(new Error('MCP server connection timeout')), MCP_TIMEOUT)
      })

      // 获取 MCP 工具，带超时处理
      const tools = await Promise.race([mcpClient.getTools(), timeoutPromise])
