    ...
    <div className="flex flex-col h-full w-[450px] lg:w-[600px] border-l border-border bg-background shadow-xl fixed right-0 top-0 bottom-0 z-50">
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b bg-muted/30">
        <div className="flex items-center gap-3">
            ...
            <div className="text-xs text-muted-foreground px-2 py-0.5 bg-muted rounded-full">v{activeArtifact.currentVersion}</div>
        </div>
        <button onClick={() => setIsCanvasVisible(false)} className="p-2 hover:bg-muted rounded-md"><X className="w-4 h-4" /></button>
      </div>
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b">
         ...
         <div className="flex gap-1">
             <button onClick={() => navigator.clipboard.writeText(code)} className="p-2 hover:bg-muted rounded-md" title="Copy"><Copy className="w-4 h-4" /></button>
         </div>
      </div>
